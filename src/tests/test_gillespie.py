...
class TestStochasticPropensity:
    ...
    def test_rate_constant_scales_linear_propensity(self):
        fast = _stochastic_propensity(
            np.array([5.0, 0.0]), np.array([[1.0, 0.0]]), np.array([3.0])
        )
        slow = _stochastic_propensity(
            np.array([5.0, 0.0]), np.array([[1.0, 0.0]]), np.array([1.0])
        )
...
class TestGillespieSimulatorTrajectory:
    ...
    def test_starts_at_initial_populations(self):
        result = GillespieSimulator(_decay_system()).run(np.array([50.0, 0.0]), t_end=1.0)
        assert result.values[0].tolist() == [50.0, 0.0]

    def test_populations_stay_non_negative(self):
        result = GillespieSimulator(_decay_system()).run(np.array([50.0, 0.0]), t_end=5.0)
...
class TestGillespieSimulatorPhysics:
    ...
    def test_equilibrated_system_stops(self):
        ...
        result = GillespieSimulator(_decay_system()).run(np.array([0.0, 50.0]), t_end=3.0)
...
class TestGillespieSimulatorSeeding:
    def test_same_seed_is_reproducible(self):
        sys = _decay_system()
        r1 = GillespieSimulator(sys).run(np.array([50.0, 0.0]), t_end=2.0, seed=7)
        r2 = GillespieSimulator(sys).run(np.array([50.0, 0.0]), t_end=2.0, seed=7)
        assert np.array_equal(r1.values, r2.values)

    def test_different_seeds_differ(self):
        sys = _decay_system()
        r1 = GillespieSimulator(sys).run(np.array([50.0, 0.0]), t_end=2.0, seed=7)
        r2 = GillespieSimulator(sys).run(np.array([50.0, 0.0]), t_end=2.0, seed=8)
